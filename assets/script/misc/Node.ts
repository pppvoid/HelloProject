...
export class NodeUtil {
    static CopyNode(node: cc.Node | cc.Prefab, parentNode: cc.Node = null): cc.Node {
        let copyNode = cc.instantiate(node) as cc.Node;
    ...
    static ResizeContents<T extends cc.Component>(content: cc.Node, newSize: number, addComType?: { prototype: T }) {
        let contentChildCount = content.children.length;
        let firstChild = content.children[0];
        if (undefined !== addComType) {
            let component = firstChild.getComponent(addComType);
            if (null === component) {
        ...
        if (contentChildCount !== newSize) {
            let layout = content.getComponent<cc.Layout>(cc.Layout);
            layout.updateLayout();
        }
    }
